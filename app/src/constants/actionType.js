// import keyMirror from 'keyMirror'

// export default keyMirror({
//     LOADED_ITEMS: null,
//     LOADED_ITEMS_DETAIL: null,
//     LOADED_ITEM_USER: null,
//     LOAD_CHART: null,
//     LOAD_TEST: null,
//     LOAD_NAVS: null,
//     LOGIN_SUCCESS: null
// })

//登录成功
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS'

//退出登录
export const LOGOUT_USER = 'LOGOUT_USER'

//显示错误信息
export const SHOW_ERROR_MSG = 'SHOW_ERROR_MSG'

//获取用户信息
export const GET_USERINFO = 'GET_USERINFO'
export const GET_USERINFO_SUCCESS = 'GET_USERINFO_SUCCESS'
export const GET_USERINFO_FAILURE = 'GET_USERINFO_FAILURE'

//加载最新公告列表
export const LOAD_NEW_LIST = 'LOAD_NEW_LIST'
export const LOAD_NEW_LIST_SUCCESS = 'LOAD_NEW_LIST_SUCCESS'

//加载最新公告列表-zhq
export const NEW_NOTICE = 'NEW_NOTICE'
export const NEW_NOTICE_SUCCESS = 'NEW_NOTICE_SUCCESS'
export const NEW_NOTICE_FAILURE = 'NEW_NOTICE_FAILURE'

//加载最新图书
export const NEW_BOOKS = 'NEW_BOOKS'
export const NEW_BOOKS_SUCCESS = 'NEW_BOOKS_SUCCESS'
export const NEW_BOOKS_FAILURE = 'NEW_BOOKS_FAILURE'

//加载图书详情
export const BOOKS_DETAIL = 'NEW_BOOKS'
export const BOOKS_DETAIL_SUCCESS = 'NEW_DETAIL_SUCCESS'
export const BOOKS_DETAIL_FAILURE = 'NEW_DETAIL_FAILURE'

//加载导航数据
export const LOAD_NAVS = 'LOAD_NAVS'
export const LOAD_NAVS_SUCCESS = 'LOAD_NAVS_SUCCESS'
export const LOAD_NAVS_FAILURE = 'LOAD_NAVS_FAILURE'

//上传Excel文件
export const UPLOAD_FILE = 'UPLOAD_FILE'
export const UPLOAD_FILE_SUCCESS = 'UPLOAD_FILE_SUCCESS'
export const UPLOAD_FILE_FAILURE = 'UPLOAD_FILE_FAILURE'

// 公司公告列表
export const NOTICE_LIST = 'NOTICE_LIST'
export const NOTICE_LIST_SUCCESS = 'NOTICE_LIST_SUCCESS'
export const NOTICE_LIST_FAILURE = 'NOTICE_LIST_FAILURE'

// 公司公告详情
export const NOTICE_DETAILS = 'NOTICE_DETAILS'
export const NOTICE_DETAILS_SUCCESS = 'NOTICE_DETAILS_SUCCESS'
export const NOTICE_DETAILS_FAILURE = 'NOTICE_DETAILS_FAILURE'

//待办事项
export const LOAD_TODO = 'LOAD_TODO'
export const LOAD_TODO_SUCCESS = 'LOAD_TODO_SUCCESS'

